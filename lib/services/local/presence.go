...
func (s *PresenceService) initSemaphore(ctx context.Context, key []byte, req services.AcquireSemaphoreRequest) (*services.SemaphoreLease, error) {
...
func (s *PresenceService) acquireSemaphore(ctx context.Context, key []byte, req services.AcquireSemaphoreRequest) (*services.SemaphoreLease, error) {
...
func (s *PresenceService) DeleteSemaphores(ctx context.Context, filter services.SemaphoreFilter) error {
	if filter.SemaphoreKind == "" {
		return trace.BadParameter("semaphore kind must be specified for deletion")
	}
	if filter.SemaphoreName != "" {
		// special case: filter corresponds to a single semaphore
